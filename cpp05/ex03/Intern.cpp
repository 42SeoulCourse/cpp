...
Form *Intern::makeForm(const std::string &name, const std::string &target) {
  static Form *(Intern::*func[3])(const std::string &) = {
      &Intern::presidentialPardonForm, &Intern::robotomyRequestForm,
      &Intern::shrubberyCreationForm};
  static std::string formName[3] = {"presidential pardon", "robotomy request",
                                    "shrubbery creation"};
  try {
    for (int i = 0; i < 3; i++) {
...
const char *Intern::IllegalInputException::what() const throw() {
  return ("❗️Intern exception❗️ Wrong Form name !!!!");
}